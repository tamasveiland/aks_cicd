...
param vnet1Name string = 'vnet-hub'

@description('Virtual network IP address.')
param vnet1AddressPrefix string = '10.0.0.0/16'

@description('Name of the second virtual network.')
param vnet2Name string = 'vnet-spoke'

@description('Virtual network IP address.')
param vnet2AddressPrefix string = '10.1.0.0/16'

// https://learn.microsoft.com/en-us/azure/azure-resource-manager/bicep/quickstart-loops?tabs=CLI#loop-with-condition
// @description('Specify if an App Service shall be provisioned.')
...
resource vnet_hub 'Microsoft.Network/virtualNetworks@2021-08-01' = {
  name: vnet1Name
  location: location
  properties: {
    addressSpace: {
    ...
    subnets: [for (sn, index) in subnets1: {
      name: sn.name
      properties: {
  ...
  resource subnets 'subnets' existing = [for item in subnets1: {
    name: item.name
  }]
...
// module vnetModule2 './modules/vnet.bicep' = {
//   name:'vnet2'
//   params: {
//     vnetName: vnet2Name
...
resource vnet_spoke 'Microsoft.Network/virtualNetworks@2021-08-01' = {
  name: vnet2Name
  location: location
  properties: {
    addressSpace: {
    ...
    subnets: [for (sn, index) in subnets2: {
      name: sn.name
      properties: {
        addressPrefix: sn.subnetPrefix
        privateEndpointNetworkPolicies: 'Disabled'
  ...
  resource subnets_spoke 'subnets' existing = [for item in subnets2: {
    name: item.name
  }]
...
module peerFirstVnetSecondVnet './modules/vnet_peering.bicep' = {
  ...
  params: {
    ...
    remoteAddressSpace: vnet2AddressPrefix
  }
  dependsOn: [
    vnet_hub
    vnet_spoke
...
module peerSecondVnetFirstVnet './modules/vnet_peering.bicep' = {
  ...
  params: {
    ...
    existingRemoteVirtualNetworkResourceGroupName: resourceGroup().name
    remoteAddressSpace: vnet1AddressPrefix
  }
  dependsOn: [
    vnet_hub
    vnet_spoke
...
module dnsZone './modules/dnsZoneRbac.bicep' = if (!empty(dnsZoneId)) {
...
module kv './modules/keyvault.bicep' = if(keyVaultCreate) {
...
module kvRbac './modules/keyvaultrbac.bicep' = if (keyVaultCreate) {
...
module kvKms './modules/keyvault.bicep' = if(keyVaultKmsCreateAndPrereqs) {
...
module kvKmsCreatedRbac './modules/keyvaultrbac.bicep' = if(keyVaultKmsCreateAndPrereqs) {
...
module kvKmsByoRbac './modules/keyvaultrbac.bicep' = if(!empty(keyVaultKmsByoKeyId)) {
...
module kvKmsKey './modules/keyvaultkey.bicep' = if(keyVaultKmsCreateAndPrereqs) {
...
module acrPool './modules/acragentpool.bicep' = if (custom_vnet && (!empty(registries_sku)) && privateLinks && acrPrivatePool) {
...
module firewall './modules/firewall.bicep' = if (azureFirewalls && custom_vnet) {
...
module userNodePool './modules/aksagentpool.bicep' = if (!JustUseSystemPool){
...
module privateDnsZoneRbac './modules/dnsZoneRbac.bicep' = if (enablePrivateCluster && !empty(dnsApiPrivateZoneId) && createAksUai) {
...
module aksmetricalerts './modules/aksmetricalerts.bicep' = if (createLaw) {
